...
class MyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if "/metrics" in self.path:
            ...
            r=requests.get("https://app.yodeck.com/api/v1/device/?username=[YOUR USERNAME]&api_key=[YOUR API KEY]&format=json")
    
        
            jsonObject = json.loads(r.text)

            for count,yodeckDevice in enumerate(jsonObject["objects"]):
                exportString = ('yodeck_device_status{{ID="{}",name="{}",IsOnline="{}"}}\n').format(yodeckDevice['id'],yodeckDevice["name"],yodeckDevice["online"])
                #print(exportString)
